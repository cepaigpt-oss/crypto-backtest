"""
Backtest: Small-cap 100% Exploders Continuation Strategy
...
def get_top_coins(limit=500):
    """Return top N coins by market cap from CoinGecko."""
...
def backtest():
    ...
    for c in tqdm(coins, desc="Coins"):
        cid = c["id"]
        ...
        price_map[cid] = df[["date", "close", "atr", "mcap", "vol", "vol_ma10"]].copy()
    ...
    for d in tqdm(all_dates, desc="Backtest"):
        ...
        for pos in open_positions:
            ...
            if row.empty:
                still_open.append(pos)
                continue
            px = float(row["close"].values[0]) * (1 - FEE_PER_SIDE)
            atr = float(row["atr"].values[0]) if not math.isnan(row["atr"].values[0]) else pos["atr"]
            pos["trail_ref"] = max(pos["trail_ref"], px)
            trail_stop = pos["trail_ref"] - ATR_MULT_STOP * atr
            exit_stop = max(trail_stop, pos["stop"])
            exit_now, reason = False, ""
            if px <= exit_stop:
                exit_now, reason = True, "stop/trail"
            elif pos["bars_held"] + 1 >= pos["max_hold"]:
                exit_now, reason = True, "time"
            pos["bars_held"] += 1
            if exit_now:
                proceeds = px * pos["qty"] * (1 - FEE_PER_SIDE)
                entry_cost = pos["entry_px"] * pos["qty"]
    ...
    if open_positions:
        ...
        for pos in open_positions:
            ...
            if df is None:
                continue
            last_row = df[df["date"] == last_day]
            if not last_row.empty:
                px = float(last_row["close"].values[0]) * (1 - FEE_PER_SIDE)
                proceeds = px * pos["qty"] * (1 - FEE_PER_SIDE)
                entry_cost = pos["entry_px"] * pos["qty"]
                pnl = proceeds - entry_cost
                equity += pnl
    ...
    if not trades_df.empty:
        ...
        print(f"Trades: {len(trades_df)} | Win%: {win_rate:.1f}% | Avg Win: ${avg_win:,.2f} | Avg Loss: ${avg_loss:,.2f}")
    else:
